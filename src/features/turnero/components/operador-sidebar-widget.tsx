...
export function OperadorSidebarWidget() {
  ...
  return (
    <div className="mx-3 mb-2 overflow-hidden rounded-xl border border-neutral-800 bg-[#111111] text-white shadow-sm">
      <a
        href="/turnero/operador"
        className="flex items-center justify-between gap-2 border-b border-neutral-800 px-3 py-2 hover:bg-[#1a1a1a]"
      >
        <div className="min-w-0">
          <p className="truncate text-[10px] font-extrabold uppercase tracking-[.2em] text-[#f97316]">
            Operador · {etiqueta}
          </p>
      ...
      <div className="space-y-2 p-3">
        <div className="rounded-lg border border-neutral-800 bg-[#0a0a0a] p-2.5">
          <p className="text-[10px] font-extrabold uppercase tracking-wider text-neutral-500">
            Turno en atención
          </p>
          {turnoActual ? (
            <>
              <p className="mt-1 break-words text-center text-sm font-extrabold leading-snug text-[#f97316]">
                {turnoActual.codigo}
              </p>
              <button
                type="button"
                disabled={cargando}
                onClick={finalizar}
                className="mt-2 w-full rounded-md bg-[#f97316] px-2 py-1.5 text-xs font-extrabold text-black hover:bg-[#fb923c] disabled:opacity-50"
              >
                Finalizar
              </button>
            </>
            ...
            <>
              <p className="mt-1 text-center text-xs font-bold text-neutral-500">
                Puesto disponible
              </p>
              <button
                type="button"
                ...
                className="mt-2 w-full rounded-md bg-[#f97316] px-2 py-1.5 text-xs font-extrabold text-black hover:bg-[#fb923c] disabled:cursor-not-allowed disabled:bg-neutral-700 disabled:text-neutral-400"
              >
        ...
        <div className="rounded-lg border border-neutral-800 bg-[#1a1a1a] p-2.5">
          <p className="text-[10px] font-extrabold uppercase tracking-wider text-[#f97316]">
            Cola de {etiqueta}
          </p>
          ...
          {espera.length > 0 && (
            <div className="mt-2 flex flex-wrap gap-1 border-t border-neutral-700 pt-2">
              {espera.slice(0, 3).map((turno) => (
                <span
                  key={turno.id}
                  className="max-w-full truncate rounded bg-[#0a0a0a] px-1.5 py-0.5 text-[10px] font-bold text-[#f97316]"
                  title={turno.codigo}
                >
                  {turno.codigo}
                </span>
