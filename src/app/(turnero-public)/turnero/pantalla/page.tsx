...
export default function PantallaPage() {
  ...
  return (
    <main className="flex min-h-dvh flex-col overflow-hidden bg-[#0a0a0a] text-white">
      <header className="flex items-center justify-between border-b border-neutral-800 bg-[#111111] px-6 py-4 sm:px-10">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-xs font-bold uppercase tracking-[.28em] text-[#f97316]">
              Turnos
            </p>
          </div>
        </div>
        <div className="flex items-center gap-4">
          ...
          {!audioActivo && (
            <button
              ...
              className="rounded-xl bg-[#f97316] px-4 py-3 font-extrabold text-black shadow-lg hover:bg-[#fb923c]"
            >
              Activar audio
            </button>
          )}
          <time className="min-w-32 text-right text-xl font-bold text-neutral-200 sm:text-3xl">
      ...
      <div className="grid flex-1 lg:grid-cols-[minmax(0,1.65fr)_minmax(340px,.75fr)]">
        <section className="flex min-h-[58vh] items-center justify-center p-6 sm:p-10">
          {actual ? (
            ...
            >
              <p className="text-2xl font-bold uppercase tracking-[.22em] text-neutral-400 sm:text-4xl">
                Turno
              </p>
              <p className="my-5 break-words px-2 text-[clamp(2.25rem,6.5vw,5.5rem)] font-extrabold leading-tight tracking-tight text-white">
                {actual.codigo}
              </p>
              <div className="mx-auto max-w-4xl rounded-3xl bg-[#f97316] px-6 py-5 text-black shadow-2xl sm:px-10 sm:py-8">
            ...
            <div className="text-center">
              ...
              <p className="text-5xl font-bold text-neutral-200 sm:text-7xl">
                Bienvenidos
              </p>
              <p className="mt-5 text-2xl text-neutral-500">Aguarde a ser llamado</p>
            </div>
          )}
        </section>

        <aside className="border-t border-neutral-800 bg-[#111111] p-6 sm:p-8 lg:border-l lg:border-t-0">
          <h2 className="mb-6 text-2xl font-extrabold uppercase tracking-wider text-neutral-400">
            Últimos llamados
          </h2>
            ...
            {historial.length ? (
              historial.map((turno) => (
                <div
                  ...
                  className="flex items-center justify-between rounded-2xl border border-neutral-800 bg-black/60 p-5"
                >
                  <span className="min-w-0 flex-1 break-words text-xl font-extrabold leading-tight sm:text-2xl">
                    {turno.codigo}
                  </span>
                  <span className="ml-4 text-right text-lg font-bold text-[#f97316] sm:text-xl">
                    {turno.puesto}
                  </span>
                </div>
              ))
            ) : (
              <p className="rounded-2xl border border-dashed border-neutral-700 p-6 text-center text-lg text-neutral-500">
                Aún no hay llamados
              </p>
            )}
          </div>
