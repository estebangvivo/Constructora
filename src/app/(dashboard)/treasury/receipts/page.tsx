...
import { listReceipts } from "@/features/treasury/queries/list-treasury";
import {
  formatMoney,
    ...
    <div className="px-4 py-6 lg:px-6">
      <div className="mb-8 flex flex-wrap items-end justify-between gap-4">
        <div>
          ...
          <h1 className="font-display text-3xl tracking-tight">Recibos</h1>
        </div>
        <Link
      ...
      {receipts.length === 0 ? (
        ...
        <ul className="divide-y divide-border border-y border-border">
          {receipts.map((item) => (
            <li key={item.id}>
              <Link
                href={`/treasury/receipts/${item.id}`}
